### 声明
    ...
    人格保证，没有任何恶意代码，不会偷key或者泄露key！！！
    可以在部署的服务器上安装抓包工具，分析每个对外的请求。

### 版本
#### 无后台：
- 版本号：3.7.12，更新日期：2024.03.22，（arm64版本号：3.7.12-arm）
- 社区版(免费版)：3.7.12-ce，更新日期：2024.03.22，免费使用，不需要授权码，功能有限制，详细看后面说明。
...
- **四**. **多模态支持**，强烈建议配置第二点说的文件上传功能：
  ...
  - 5.**claude-3**，可以上传图片。目前没有实现直接调用官方接口，需要通过中转接口实现，这边发出的报文跟gpt4v一样格式。非图片文件会把文件链接拼在内容里。
- **五**. **逆向模型支持**
